/* OfficeFlow Pro v2 — OfficeFlow-backed workspace */
(() => {
  const $ = (s, r = document) => r.querySelector(s);
  const $$ = (s, r = document) => [...r.querySelectorAll(s)];
  const configured = true;
  const sb = officeflow.createClient();
  ...
  async function initApp(){
    ...
    if(!configured){ gate?.classList.add('hidden'); openModal(`<div class="eyebrow">DATABASE SETUP</div><h2 class="modal-title">OfficeFlow database is built in</h2><p class="modal-sub">The UI is ready. Start the Node server. OfficeFlow creates its own database and private file storage automatically.</p><div class="setup-inline">Keep OFFICEFLOW_AUTH_SECRET on the server only.</div><div class="modal-actions"><button class="btn primary" id="goLogin">Open sign in / register</button></div>`); $('#goLogin').onclick=()=>location.href='login.html'; return; }
