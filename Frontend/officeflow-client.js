  ...
  function createClient() {
    ...
    return {
      ...
      auth: {
        ...
        async resetPasswordForEmail(email, options = {}) { return parse(await fetch('/api/auth/password-reset', { method: 'POST', headers: jsonHeaders(), body: JSON.stringify({ email, redirectTo: options.redirectTo }) })); },
        async resetPassword(token, password) { const out = await parse(await fetch('/api/auth/password-reset/confirm', { method: 'POST', headers: jsonHeaders(), body: JSON.stringify({ token, password }) })); if (!out.error && out.data?.session?.access_token) localStorage.setItem(tokenKey, out.data.session.access_token); return out; }
      },
      storage: { from: bucket => ({
